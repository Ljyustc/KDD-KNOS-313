...
def convert_formula_pre2infix(formula):
    stack = [] 
    op_list = ['+','-','*','/','^']
    for i in range(2, len(formula)):
        stack.append(formula[i])
        if formula[i] in op_list:
            continue
        else:
            while len(stack) >= 3 and stack[-3] in op_list and stack[-2] not in op_list and stack[-1] not in op_list:
                right_operand = stack.pop()
                left_operand = stack.pop()
                op = stack.pop()
                op_result = "("+left_operand+op+right_operand+")"
                stack.append(op_result)
    if len(stack) != 1:
        print("error")
    result = formula[0]+formula[1]+stack[0]
    return result