...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, evi_batch, evi_length, evi_num_conv, step_batch, step_length, step_label_batch, generate_nums,
               encoder, encoder2, predict, generate, merge, encoder_optimizer, encoder2_optimizer, predict_optimizer, generate_optimizer,
    ...
    evi_seq_mask = []
    max_evi_len = max(evi_length)
    for i in evi_length:
        evi_seq_mask.append([0 for _ in range(i)] + [1 for _ in range(i, max_evi_len)])
    evi_seq_mask = torch.BoolTensor(evi_seq_mask)
    
    step_masks = []
    for step in step_length:
        step_mask = []
        max_step_len = max(step)
        for i in step:
            step_mask.append([0 for _ in range(i)] + [1 for _ in range(i, max_step_len)])
        step_masks.append(torch.BoolTensor(step_mask))
    ...
    target = torch.LongTensor(target_batch).transpose(0, 1)
    evi_var = torch.LongTensor(evi_batch)
    step_label_batch = torch.LongTensor(step_label_batch)
    ...
    evi_num_conv_ = torch.LongTensor(evi_num_conv)
    
    if USE_CUDA:
        input_var = input_var.cuda()
        evi_var = evi_var.cuda()
        seq_mask = seq_mask.cuda()
        evi_seq_mask = evi_seq_mask.cuda()
        padding_hidden = padding_hidden.cuda()
        num_mask = num_mask.cuda()
        evi_num_conv_ = evi_num_conv_.cuda()
    ...
    # Run words through encoder
    encoder_outputs, problem_output = encoder(input_ids=input_var,attention_mask=1 - seq_mask.to(torch.long))
    ...
    # evi_outputs, evi_output = encoder2(input_ids=evi_var,attention_mask=1 - evi_seq_mask.to(torch.long),evi_num_conv=evi_num_conv_,pro_outputs=encoder_outputs)

    # Prepare input and output variables
    # problem_output = problem_output + evi_output
    ...
    for t in range(max_target_length):
        step_var = torch.LongTensor(step_batch[t]).cuda()
        step_mask = torch.BoolTensor(step_masks[t]).cuda()
        _, step_output = encoder2(input_ids=step_var,attention_mask=1 - step_mask.to(torch.long))
        
        num_score, op, current_embeddings, current_context, current_nums_embeddings, verify_prob = predict(
            node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask, step_output)

        # all_leafs.append(p_leaf)
        outputs = torch.cat((op, num_score), 1)
        all_node_outputs.append(outputs)
        all_verify_outputs.append(verify_prob)
    ...
    all_verify_outputs = torch.stack(all_verify_outputs, dim=1)

    target = target.transpose(0, 1).contiguous()
    if USE_CUDA:
        # all_leafs = all_leafs.cuda()
        all_node_outputs = all_node_outputs.cuda()
        all_verify_outputs = all_verify_outputs.cuda()
        target = target.cuda()
        step_label_batch = step_label_batch.cuda()

    # op_target = target < num_start
    # loss_0 = masked_cross_entropy_without_logit(all_leafs, op_target.long(), target_length)
    verify_loss = masked_cross_entropy_without_logit(all_verify_outputs, step_label_batch, target_length)
    loss = masked_cross_entropy(all_node_outputs, target, target_length) + 0.1 * verify_loss
    # loss = loss_0 + loss_1
    loss.backward()
    # clip the grad
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, encoder2, predict, generate, merge, formula_enc, verify, input_lang, output_lang, num_pos, evi_batch, evi_length, evi_num_conv, 
    ...
    evi_var = torch.LongTensor(evi_batch).unsqueeze(0)
    ...
    # Run words through encoder
    encoder_outputs, problem_output = encoder(input_ids=input_var)

    num_size = len(num_pos)
    all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, [num_pos], batch_size, num_size,
                                                              encoder.hidden_size)
                                                              
    # evi_outputs, evi_output = encoder2(input_ids=evi_var,evi_num_conv=evi_num_conv_,pro_outputs=encoder_outputs)
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings, formula_prob, verify_prob = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden,
                seq_mask, num_mask, formula_scores, formula_roots, formula_none_id, verify)
...
try_cnt = 0
def get_API_res(question, ww_know, wo_know, formula_prompt, nums, model_set='GPT3', know=False):
    global try_cnt
    api_key_nums = len(api_keys)
    try_cnt = (try_cnt + 1) % api_key_nums
    count = 0
    if len(ww_know) == 0:
        ww_knows = ''
    else:
        ww_knows = "knowledge among words: " + ww_know + ', '
    if len(wo_know) == 0:
        wo_knows = ''
    else:
        wo_knows = "knowledge between words and operators: " + wo_know + ', '
    if formula_prompt == 'None':
        for_knows = ''
    else:
        for_knows = 'knowledge of formulas: ' + formula_prompt + ", "
    while True:
        config = configure()
        model = GPT3(config)
        if know == True:
            prompt = "Given problem: \"" + question + "\", " + ww_knows + wo_knows + for_knows + 'tell me how to complete a Prefix expression and explain how you apply the knowledge to get the result: \'answer = ['
        else:
            prompt = "Given problem: \"" + question + "\", " + 'tell me how to complete a Prefix expression and explain the reason: \'answer = ['
        prompt_s = prompt + " ".join(nums)
        sel_key_id = (try_cnt + count) % api_key_nums
        sel_key = api_keys[sel_key_id]
        openai.api_key = sel_key
        #设置openai网址 api_bases随机选择一个
        openai.api_base = api_bases[random.randint(0, len(api_bases) - 1)]
        try:
            if model_set == 'GPT3':
                res = model.ask_gpt(prompt_s)
            else:
                res = model.ask_chatgpt(prompt_s)
        ...
        if res is not None:
            ...
            if count > api_key_nums * 2 :
                print("The openai api cannot work! we will return none")
                break
            count += 1
            time.sleep(min(1024,  2 ** count))
            
    try_cnt = sel_key_id
    return res

def evaluate_tree_GPT3(input_batch, input_length, generate_nums, encoder, encoder2, predict, generate, merge, formula_enc, verify, input_lang, output_lang, num_pos, evi_batch, evi_length, evi_num_conv, nums_list, question, ww_know, wo_know, formulas, tokenizer,
                  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    # evi_outputs, evi_output = encoder2(input_ids=evi_var)
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            # select formula
            _, _, _, _, _, formula_prob, _ = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden,
                seq_mask, num_mask, formula_scores, formula_roots, formula_none_id, verify)
            formula_select = int(torch.argmax(formula_prob[0]))
            if formula_select != formula_none_id:
                formula_prefix = formulas[formula_select]
                formula_prompt = convert_formula_pre2infix(formula_prefix)
            else:
                formula_prompt = 'None'

            step_nums = out_expression_list(b.out, output_lang, nums_list)
            step_out = get_API_res(question, ww_know, wo_know, formula_prompt, step_nums, model_set='ChatGPT')
            step_output, step_outputs = None, None
            if step_out is not None:
                # logging.info("question: "+question+" step_nums: "+str(step_nums)+" API: "+step_out)
                step_out_cells = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(step_out))
                step_var = torch.LongTensor(step_out_cells).cuda()
                step_var = step_var.unsqueeze(0)
                step_outputs, step_output = encoder2(input_ids=step_var)
            else:
                logging.info("question: "+question+" step_nums: "+str(step_nums)+" API: "+"openai api error!")

            num_score, op, current_embeddings, current_context, current_nums_embeddings, _, verify_prob = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden,
                seq_mask, num_mask, formula_scores, formula_roots, formula_none_id, verify, step_output=step_output, step_outputs=step_outputs)
...
def train_verifier(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, evi_batch, evi_length, evi_num_conv, step_batch, step_length, step_label_batch, generate_nums,
               encoder, encoder2, predict, generate, merge, formula_enc, verify, encoder_optimizer, encoder2_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, formula_enc_optimizer, verify_optimizer, input_lang, output_lang, num_pos, formula_batch, english=False):
    # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    for i in input_length:
    ...
    # evi_outputs, evi_output = encoder2(input_ids=evi_var,attention_mask=1 - evi_seq_mask.to(torch.long))
    ...
    for t in range(max_target_length):
        step_var = torch.LongTensor(step_batch[t]).cuda()
        step_mask = torch.BoolTensor(step_masks[t]).cuda()
        step_label = torch.LongTensor(step_label_batch[:,[t]]).cuda()
        step_outputs, step_output = encoder2(input_ids=step_var,attention_mask=1 - step_mask.to(torch.long))
        
        num_score, op, current_embeddings, current_context, current_nums_embeddings, formula_prob, verify_prob = predict(
            node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask, formula_scores, formula_roots, formula_none_id, verify, step_output, step_outputs, step_label, step_mask, train=True, formula_gt=formula_batch[:,t])
    ...
    verify_loss = masked_cross_entropy_without_logit(all_verify_outputs, step_label_batch, target_length)
    loss = masked_cross_entropy(all_node_outputs, target, target_length) + 0.1 * verify_loss + 0.05 * formula_pre_loss
    # loss = loss_0 + loss_1
    loss.backward()
    # clip the grad
    # torch.nn.utils.clip_grad_norm_(encoder.parameters(), 5)
...
def evaluate_verifier(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, evi_batch, evi_length, evi_num_conv, step_batch, step_length, step_label_batch, generate_nums,
               encoder, encoder2, predict, generate, merge, formula_enc, verify, input_lang,output_lang, num_pos,english=False):
    # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    for i in input_length:
    ...
    for t in range(max_target_length):
        ...
        num_score, op, current_embeddings, current_context, current_nums_embeddings, formula_prob, verify_prob = predict(
            node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask, formula_scores, formula_roots, formula_none_id, verify, step_output, step_outputs)
    ...
    if USE_CUDA:
        all_verify_outputs = all_verify_outputs.cuda()
        step_label_batch = step_label_batch.cuda()
    acc_con, acc_all, pre_rec_con, pre_all, rec_all = masked_acc_without_logit(all_verify_outputs, step_label_batch, target_length)
    return acc_con, acc_all, pre_rec_con, pre_all, rec_all
