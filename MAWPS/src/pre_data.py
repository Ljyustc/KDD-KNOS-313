...
def transfer_num1(data,tokenizer=None):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
    ...
    copy_nums = 0
    for d in data:
        #-----Encode problem sentence----
        nums = []
        input_seq = []
        seg = ("question: " + d["question"].strip()).split(" ")
        ...
        for s in seg:
            ...
            if pos and pos.start() == 0:
                nums.append(s[pos.start(): pos.end()])
                num_value = compute_postfix_expression([s[pos.start(): pos.end()]])
                num2id[num_value] = num_id
                seg1.append('[NUM]')
                num_id += 1
                if pos.end() < len(s):
                    temp = s[pos.end():]
                ...
                seg1.append(s)
        sen = ' '.join(seg1)
        input_seq = tokenizer.tokenize(sen)
        #-------------------------------

        #-------Encode evidence---------
        seg_evi = d["GPT3_explanation"].strip().split(" ")
        seg1_evi = [] 
        evi_id2num = {} 
        evi_num_id = 0
        for s in seg_evi:
            ...
            if pos and pos.start() == 0:
                seg1_evi.append('[NUM]')
                evi_id2num[evi_num_id] = s[pos.start(): pos.end()]
                evi_num_id += 1
                if pos.end() < len(s):
                    temp = s[pos.end():]
                    seg1_evi.append(temp)
            else:
                seg1_evi.append(s)
        sen_evi = ' '.join(seg1_evi)
        input_evi = tokenizer.tokenize(sen_evi)
        #--------------------------------------

        #------Replace number in evidence------
        inp_id2pos = {}
        num_count = 0
        for word_pos in range(len(input_seq)):
            if input_seq[word_pos] == '[NUM]':
                inp_id2pos[num_count] = word_pos
                num_count += 1
        
        # 分词后的evidence_seq中，数值应替换为input_seq的位置，-1代表无需替换
        evi_num_conv = [-1]*len(input_evi)
        num_count = 0
        for word_pos in range(len(input_evi)):
            if input_evi[word_pos] == '[NUM]':
                num_value = compute_postfix_expression([evi_id2num[num_count]])
                for n in num2id:
                    if n == num_value:
                        evi_num_conv[word_pos] = inp_id2pos[num2id[n]]
                        break
                num_count += 1
        #--------------------------------------
        ...
        #----------Encode GPT3 steps-----------
        GPT3_steps = []
        for seg_step in d["GPT3_steps"]:
            input_step = tokenizer.tokenize(seg_step)
            GPT3_steps.append(input_step)
        step_labels = d["step_label"]
        #--------------------------------------
        ...
        assert len(nums) == len(num_pos)
        # pairs.append((input_seq, out_seq, nums, num_pos, d["ans"]))
        pairs.append((input_seq, out_seq, nums, num_pos, input_evi, evi_num_conv, GPT3_steps, step_labels))
...
def prepare_data1(pairs_trained, pairs_tested,trim_min_count, generate_nums, copy_nums,tokenizer,formula_exp_dict,tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        num_stack.reverse()
        input_cell = tokenizer.convert_tokens_to_ids(pair[0])
        
        input_evi_cell = tokenizer.convert_tokens_to_ids(pair[-5])
        input_evi_num_conv = pair[-4]
        
        input_steps_cell = []
        for step in pair[-3]:
            input_step = tokenizer.convert_tokens_to_ids(step)
            input_steps_cell.append(input_step)
        
        output_cell = indexes_from_sentence(output_lang, pair[1], tree)
        formula_id = [formula_exp_dict[item] for item in pair[-1]]
        train_pairs.append((input_cell,len(input_cell), output_cell, len(output_cell),
                            pair[2], pair[3], num_stack, input_evi_cell, len(input_evi_cell), input_evi_num_conv, input_steps_cell, [len(x) for x in input_steps_cell], pair[-2], formula_id))
    print('Indexed %d words in input language, %d words in output' % (input_lang.n_words, output_lang.n_words))
    print('Number of training data %d' % (len(train_pairs)))
    for pair in pairs_tested:
        num_stack = []
...
#pairs:input_seq, len(input_seq), out_seq(prefix with index), len(out_seq), nums, num_pos, num_stack
    for batch in batches:
        ...
        evi_length = []
        step_length = [[]]
        for _, i, _, j, _, _, _, _, k, _, _, ms, _, _ in batch:
            input_length.append(i)
            output_length.append(j)
            ...
            for m in range(len(ms)):
                if m+1 > len(step_length):
                    step_length.append([0]*(len(step_length[-1])-1))
                step_length[m].append(ms[m])
            for m in range(len(ms), len(step_length)):
                step_length[m].append(0)

        input_lengths.append(input_length)
        output_lengths.append(output_length)

        evi_lengths.append(evi_length)
        ...
        evi_len_max = max(evi_length)
        
        step_lengths_max = [max(step_l) for step_l in step_length]
        ...
        step_batch = [[] for _ in range(len(step_lengths_max))]
        step_label = []
        ...
        num_size_batch = []
        formula_batch = []
        for i, li, j, lj, num, num_pos, num_stack, k, lk, num_conv, steps, ms, step_la, formula in batch:
            num_batch.append(len(num))
            input_batch.append(pad_seq(i, li, input_len_max))
            output_batch.append(pad_seq(j, lj, output_len_max))
  
            evi_batch.append(pad_seq(k, lk, evi_len_max))
            evi_num_conv_batch.append(num_conv+[-1 for _ in range(evi_len_max - lk)])
            
            step_label.append(pad_seq(step_la, lj, output_len_max))

            for step_id in range(len(step_lengths_max)):
                if step_id+1 > len(steps):
                    step_batch[step_id].append(pad_seq([], 0, step_lengths_max[step_id]))
                else:
                    step_batch[step_id].append(pad_seq(steps[step_id], ms[step_id], step_lengths_max[step_id]))
        ...
        evi_batches.append(evi_batch)
        evi_num_conv_batches.append(evi_num_conv_batch)

        step_batches.append(step_batch)
        step_label_batches.append(step_label)

        num_stack_batches.append(num_stack_batch)
        num_pos_batches.append(num_pos_batch)
        num_size_batches.append(num_size_batch)
        formula_batches.append(formula_batch)
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, evi_batches, evi_lengths, evi_num_conv_batches, step_batches, step_lengths, step_label_batches, formula_batches
