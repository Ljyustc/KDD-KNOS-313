...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if evi_num_conv is not None and pro_outputs is not None:
            pro_outputs_ = pro_outputs.transpose(0,1)
            #print(evi_num_conv.size(), pro_outputs_.size(), inputs_embeds.size())
            evi_num_conv_ = evi_num_conv.unsqueeze(-1).repeat(1,1,pro_outputs_.size(-1))
            rep_embeds = pro_outputs_.gather(1, evi_num_conv_.masked_fill(evi_num_conv_<0,0))
            inputs_embeds = rep_embeds*(evi_num_conv_>-1) + inputs_embeds*(evi_num_conv_==-1)
...
class T5Encoder(T5PreTrainedModel):
    ...
    Examples::
        ...
        encoder_outputs = self.encoder(
            ...
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            evi_num_conv=evi_num_conv,
            pro_outputs=pro_outputs
        )

        hidden_states = encoder_outputs[0]  # B x S x H
        problem_output = hidden_states[:,0,:] + hidden_states[:,-1,:]
        return hidden_states.transpose(0,1), problem_output
